...
$('document').ready(function () {
    ...
    optionToDom(option);
    document.title = option.page.toString();

    prev.click(function () {
        if (option.page === 1) {
            alert('当前已经是第一张');
        }
       location.href = '/?' + optionToQuery(getOptionFromDom(), 'prev');
    });
    next.click(function () {
        if (option.page === MAX_PAGE) {
            alert('当前已经是最后一张');
        }
        location.href = '/?' + optionToQuery(getOptionFromDom(), 'next');
    });
